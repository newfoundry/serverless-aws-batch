...
function buildDockerImage() {
    ...
    let additionalBuilderCommands = "";
    if (this.serverless.service.custom
        && this.serverless.service.custom.awsBatch
        && this.serverless.service.custom.awsBatch.additionalDockerBuilderCommands) {

        _.forEach(
            this.serverless.service.custom.awsBatch.additionalDockerBuilderCommands,
            runCommand => {
                additionalBuilderCommands += `${runCommand}\n`;
            }
        )
    }

    // Get any additional run commands we'd like to include in the docker image
    let additionalProductionCommands = "";
    if (this.serverless.service.custom
        && this.serverless.service.custom.awsBatch
        && this.serverless.service.custom.awsBatch.additionalDockerProductionCommands) {

        _.forEach(
            this.serverless.service.custom.awsBatch.additionalDockerProductionCommands,
            runCommand => {
                additionalProductionCommands += `${runCommand}\n`;
            }
        )
    }
